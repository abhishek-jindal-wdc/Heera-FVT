...
class HBDEI06_HybridPFOnPrimaryAndSecondary(TestCase.TestCase):
    ...
    def testHBDEI06_HybridPFOnPrimaryAndSecondary(self):
    ...
    def OnUmWrite(self , args):
        ...
        if args["MB"] not in self.errorInjectedLbaList and args["MB"] % self.__fwConfigObj.numberOfMetaPlane in self.MMPList:
            if self.injectError:
                ...
                if args["primary"] == 0x1:    #PrimaryMB                
                    #self.injectOnSecondary = True
                    self.__logger.Info(self.globalVarsObj.TAG, "...Injecting 4 Program Failures on Primary Hybrid block: 0x%X..." % args["MB"])
                
                if args["primary"] == 0x0 and self.injectOnSecondary:   #SecondaryMB  
                    # Inject PFs on 4 wordlines for block to retire considering shifted dual write from Primary
                    secondaryWL.append(self.__randomObj.randrange(self.wordlines[0]-7,(self.__fwConfigObj.wordLinesPerPhysicalBlock/2+8 - 1) - 7 +1))
                    secondaryWL.append(self.__randomObj.randrange(self.wordlines[1]-7,(self.__fwConfigObj.wordLinesPerPhysicalBlock/2+18 - 1) - 7 +1))
                    secondaryWL.append(self.__randomObj.randrange(self.wordlines[2]-7,(self.__fwConfigObj.wordLinesPerPhysicalBlock/2+28 - 1) - 7 +1))
                    secondaryWL.append(self.__randomObj.randrange(self.wordlines[3]-7,(self.__fwConfigObj.wordLinesPerPhysicalBlock/2+38 - 1) - 7 +1))
                    self.wordlines = secondaryWL
                    self.injectError = False         
                    self.injectOnSecondary = False
                    self.__logger.Info(self.globalVarsObj.TAG, "...Injecting 4 Program Failures on Secondary Hybrid block: 0x%X..." % args["MB"])
